...
import {
  Briefcase, Eye, ShieldCheck, Sparkles, ArrowUpRight,
...
function CorporateDashboardPage() {
  ...
  const stats = [
    ...
    { tone: "purple", icon: <Trophy className="size-4" />, label: "Top Learners", value: "—", sub: "join to populate" },
    { tone: "green", icon: <Briefcase className="size-4" />, label: "Active Listings", value: corp.plan ? "0" : "0", sub: corp.plan ?? "no plan yet" },
  ];

  const sampleVisits = corp.recentProfileVisits;
            ...
            <div className="glass-card p-5">
              ...
              <div className="space-y-3">
                {sampleVisits.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-border/60 bg-white/40 px-4 py-8 text-center text-xs text-muted-foreground">
                    No profile visits yet. Browse top learners to start sourcing.
                  </div>
                ) : (
                  sampleVisits.map((v) => (
                    <button
